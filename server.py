...
def get_result_resp_queue():
    while True:
        response = sqs_client.receive_message(QueueUrl=sqs_resp_queue.url, MaxNumberOfMessages=1, WaitTimeSeconds=5)
    ...
    try:
        ...
        for _ in range(10):
            if filename_no_ext in face_reg_dict:
                return f"{filename_no_ext}:{face_reg_dict.pop(filename_no_ext)}\n", 200
            time.sleep(0.5)
    except Exception as e:
